...
export default function ChatPanel({ onClose, dsTheme, roomId }: { onClose: () => void; dsTheme: DSTheme; roomId: string }) {
  ...
  return (
    <div style={{
      ...
      {/* Username bar */}
      <div style={{ padding: '4px 6px', borderBottom: `1px solid ${dsTheme.bevelDark}`, fontSize: '10px', display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, background: dsTheme.glass }}>
        <span style={{ color: dsTheme.textMuted }}>You:</span>
        {editingName ? (
          <>
            <Win95Input dsTheme={dsTheme} value={nameInput}
              onChange={e => setNameInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') saveName(); if (e.key === 'Escape') setEditingName(false); }}
              autoFocus maxLength={28} style={{ flex: 1 }} />
          ...
          <>
            ...
            <Btn dsTheme={dsTheme} variant="ghost" size="sm" style={{ marginLeft: 'auto', minWidth: 'unset' }}
              onClick={() => { setNameInput(username); setEditingName(true); }}>Edit</Btn>
          </>
        )}
      </div>

      {/* Messages */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 6px', display: 'flex', flexDirection: 'column', gap: '6px', background: dsTheme.surface, backdropFilter: dsTheme.blur, WebkitBackdropFilter: dsTheme.blur, border: `1px solid ${dsTheme.glassBorder}`, margin: '4px' }}>
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', color: dsTheme.textMuted, marginTop: '20px', fontSize: '10px', lineHeight: 1.8 }}>No messages yet.<br />Say hello! 👋</div>
        )}
        {messages.map(m => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isOwn ? 'flex-end' : 'flex-start' }}>
              ...
              <div style={{ maxWidth: '85%', padding: '8px 12px', background: isOwn ? `${dsTheme.accent1}cc` : dsTheme.glass, backdropFilter: dsTheme.blur, WebkitBackdropFilter: dsTheme.blur, color: isOwn ? '#fff' : dsTheme.text, fontSize: '11px', fontFamily: '"Space Mono", monospace', border: `1px solid ${isOwn ? dsTheme.accent1 : dsTheme.glassBorder}`, borderRadius: isOwn ? '12px 12px 2px 12px' : '12px 12px 12px 2px', wordBreak: 'break-word', lineHeight: '1.6' }}>
                {m.text}
              </div>
            </div>
          );
      ...
      <div style={{ display: 'flex', gap: '4px', padding: '4px 6px 6px', flexShrink: 0 }}>
        <Win95Input dsTheme={dsTheme} value={input}
          onChange={e => setInput(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }}
          placeholder="Type a message…" maxLength={400} style={{ flex: 1 }} />
        <Btn dsTheme={dsTheme} variant="primary" size="sm" onClick={send} disabled={!input.trim()}>Send</Btn>
      </div>
    </div>
  );
}
